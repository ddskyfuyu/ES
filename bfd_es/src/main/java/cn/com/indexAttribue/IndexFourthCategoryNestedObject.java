 ...
 * 填充四级品类信息
 */
public class IndexFourthCategoryNestedObject implements IndexNestedObject {
    ...
    public IndexFourthCategoryNestedObject(IndexFloatAttribute indexFloatAttribute, IndexStringAttribute indexStringAttribute,
    ...
    public void fillNestedObject(JSONObject obj, Map<String, Map<String, String>> value) {
        ...
        for(Map.Entry<String, Map<String, String>> entry : value.entrySet()){
            ...
            if(entry.getKey().equals("String")){
                for(Map.Entry<String, String> subEntry : entry.getValue().entrySet()){
                    if(subEntry.getKey().equals("fifth_category")){
                        continue;
                    }
